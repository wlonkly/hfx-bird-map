...
## Python Environment
- **Always use the `python` on the PATH** (managed by asdf). Never call system `/usr/local/bin/python3` or hardcode a Homebrew path — the system Python may be too old for the syntax in `fetch_bird_nests.py`.
- The script uses `str | None` annotations, which require **Python 3.10+**.
- If `pytest` is not installed for the asdf Python, run `python -m pip install pytest` first, then use `python -m pytest` to run tests.
...
python -m pytest tests/test_fetch_bird_nests.py -k TestGenerateHtml
```

## Architecture
- No package manager files (`pyproject.toml`, `requirements.txt`, etc.). The script uses only the Python stdlib (requires 3.10+ for `str | None` and `zoneinfo`).
